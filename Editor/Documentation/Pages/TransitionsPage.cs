    ...
    public sealed class TransitionsPage : IHonamiDocumentationPage
    {
        ...
        public void BuildContent(VisualElement root)
        {
            ...
            HonamiDocumentationBuilder.AddPropertyBox(root,
                ...
                (HonamiDocLocalization.Get("Easing", "Easing"), HonamiDocLocalization.Get("Shapes the blend weight without any setup: the full Penner set (Sine, Quad, Cubic, Quart, Quint, Expo, Circ, Back, Elastic, Bounce — each as In/Out/In-Out), same as tweening libraries like PrimeTween. Out Quad is a solid default for locomotion; In-Out variants hide both edges of the blend; Back and Elastic overshoot past the target pose for stylized snaps. Pick Custom Curve to hand-edit the curve instead.", "Формує вагу бленду без жодного налаштування: повний набір Пеннера (Sine, Quad, Cubic, Quart, Quint, Expo, Circ, Back, Elastic, Bounce — кожен як In/Out/In-Out), як у твін-бібліотеках на кшталт PrimeTween. Out Quad — хороший дефолт для локомоушна; In-Out-варіанти ховають обидва краї бленду; Back та Elastic перелітають цільову позу для стилізованих снапів. Оберіть Custom Curve, щоб редагувати криву вручну.")),
