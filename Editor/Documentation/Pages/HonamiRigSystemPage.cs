    ...
    public sealed class HonamiRigSystemPage : IHonamiDocumentationPage
    {
        ...
        public void BuildContent(VisualElement root)
        {
            ...
            HonamiDocumentationBuilder.AddFeatureGrid(root,
                ...
                (HonamiDocLocalization.Get("Pseudo-Physics Constraint", "Pseudo-Physics Constraint"), HonamiDocLocalization.Get("Adds springy inertia, drag and secondary motion to any list of bones.", "Додає spring inertia, drag та secondary motion до будь-якого списку кісток."), HonamiEditorIcons.TimelineWhite)
            );
...
// positionOffset/rotationOffset keep authored grip offsets.");
            ...
            HonamiDocumentationBuilder.AddParagraph(root, HonamiDocLocalization.Get(
                "Pseudo-Physics adds reactive spring motion to bones after the authored animation is sampled. It is intentionally lightweight: no Rigidbody chain, no full physics simulation, just controlled secondary motion.",
                "Pseudo-Physics додає reactive spring motion до кісток після sampling authored animation. Він навмисно легкий: без Rigidbody chain, без повної physics simulation, лише контрольований secondary motion."
            ));
            HonamiDocumentationBuilder.AddPropertyBox(root,
                (HonamiDocLocalization.Get("Good targets", "Вдалі цілі"), HonamiDocLocalization.Get("Hair, cloth strips, tails, antennae, soft armor, weapon sway, hanging props.", "Hair, cloth strips, tails, antennae, soft armor, weapon sway, hanging props.")),
                (HonamiDocLocalization.Get("Position physics", "Position physics"), HonamiDocLocalization.Get("Drag, inertia, stiffness, damping, max offset and local gravity.", "Drag, inertia, stiffness, damping, max offset та local gravity.")),
                (HonamiDocLocalization.Get("Rotation physics", "Rotation physics"), HonamiDocLocalization.Get("Angular drag/inertia with rotation stiffness, damping and max rotation.", "Angular drag/inertia з rotation stiffness, damping та max rotation.")),
                (HonamiDocLocalization.Get("Axis masks", "Axis masks"), HonamiDocLocalization.Get("Restrict motion to only the local axes that should move.", "Обмежуйте рух лише локальними осями, які мають рухатися."))
            );
